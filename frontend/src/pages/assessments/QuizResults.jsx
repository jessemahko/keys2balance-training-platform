...
import { ArrowLeft, Users, BarChart3, Edit, ChevronDown, ChevronUp } from 'lucide-react'
...
const QuizResults = () => {
	...
	const maxScore = questions.reduce(
		(sum, q) => sum + ((q.type || 'single_choice') === 'open_text' ? (q.max_points || 1) : 1),
		0,
	)

	const getResponseScore = (r) => {
		if (r.answers_json?.grading_status === 'complete' && r.answers_json?.total_score !== undefined) {
	...
	const handleGradeSubmit = async (responseId, questionId, score) => {
		...
		try {
			await assessmentService.gradeQuestion(assessmentId, responseId, questionId, score)
			const res = await assessmentService.getResults(assessmentId)
			setData(res)
		} catch (err) {
									...
									return (
										<>
											<tr
												...
												onClick={() => hasOpenText && setExpandedResponse(isExpanded ? null : r.response_id)}
											>
													...
													<td colSpan={5} className="px-6 py-4 bg-gray-50">
														<div className="space-y-4">
															{openTextQuestions.map((q) => {
																const studentAnswer = r.answers_json?.answers?.[q.id] || '(no answer)'
																const existingScore = r.answers_json?.manual_scores?.[q.id]
																const key = `${r.response_id}_${q.id}`
																return (
																	<div key={q.id} className="bg-white p-4 rounded-lg border border-border-color">
																		...
																		<div className="flex items-center gap-3">
																			...
																			<input
																				...
																				min={0}
																				max={q.max_points || 1}
																				value={gradingScores[key] ?? existingScore ?? ''}
																				onChange={(e) =>
																			...
																			<button
																				onClick={(e) => {
																					...
																					handleGradeSubmit(
																						r.response_id,
																						q.id,
																						gradingScores[key] ?? existingScore ?? 0,
																					)
																				}}
