...
const BlockEditorModal = ({ isOpen, onClose, onSave, initialData, isNew }) => {
  ...
  const inputClass = "w-full px-4 py-3 border border-border-color rounded-lg text-base font-sans transition-all bg-[#fdfdfd] focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20";
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[1000] p-5 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-[700px] max-h-[90vh] overflow-y-auto rounded-2xl shadow-xl flex flex-col animate-in slide-in-from-bottom-8 duration-300">
        <div className="p-6 md:px-8 border-b border-border-color flex justify-between items-center sticky top-0 bg-white z-10">
          <h2 className="text-xl text-primary font-bold">{isNew ? 'Add New Content Block' : 'Edit Block'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-8 flex-1 flex flex-col gap-6">
          ...
          {type === 'file_attachment' && (
            <>
              <div>
                ...
                <input type="text" name="title" value={formData.title || ''} onChange={handleChange} className={inputClass} placeholder="e.g. Course Materials" required />
              </div>
              <div>
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center text-gray-500 h-full">
                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#9484b4" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                {formData.files && formData.files.length > 0 && (
                  <ul className="mt-4 list-none p-0 flex flex-col gap-2">
                    {formData.files.map((file, idx) => (
                      <li key={idx} className="flex items-center justify-between p-3 bg-primary/5 rounded-lg border border-primary/10">
                        <span className="font-medium text-gray-800 truncate max-w-[200px] md:max-w-xs">{file.name}</span>
                        <span className="text-sm text-gray-500 ml-auto mr-4">{formatFileSize(file.size)}</span>
                        <button 
                          type="button" 
          ...
          {type === 'recording_link' && (
            <>
              <div>
                ...
                <input type="text" name="title" value={formData.title || ''} onChange={handleChange} className={inputClass} placeholder="e.g. Class Recording - Week 1" required />
              </div>
              <div>
                <label className={labelClass}>URL:</label>
                <input type="url" name="url" value={formData.url || ''} onChange={handleChange} className={inputClass} placeholder="https://example.com/recording" required />
              </div>
              <div>
                <label className={labelClass}>Description (optional):</label>
                <textarea name="description" value={formData.description || ''} onChange={handleChange} className={`${inputClass} min-h-[80px]`} rows={2} placeholder="Brief description of the link..." />
              </div>
            </>
          )}
          ...
          <div className="flex justify-end gap-3 mt-auto pt-4 border-t border-border-color">
            ...
            <button type="submit" className="inline-flex items-center justify-center bg-primary text-white px-6 py-2.5 rounded-lg border-none font-semibold cursor-pointer transition-all shadow-md hover:bg-[#3f356d] hover:shadow-lg hover:-translate-y-[1px]">
